...
class CCIPCANode(object):
    ...
    **Instance variables of interest**
    ...
    def __init__(self, input_dim, output_dim, **kwargs):
        ...
        # n value for the ccipca
        self.n = 1
        # Internal Eigen Vector (unNormalized)
        self._v = 0.1*np.random.randn(self.output_dim, self.input_dim)
        self._d = np.sum(np.absolute(self._v)**2, axis=-
                         1)**(1./2)           # Internal Eigen Values
        # Internal Eigen Vector (Normalized)
        self._vn = self._v/self._d.reshape(self._v.shape[0], 1)
        # Total Explained Variance
        self.explained_var_tot = self._d.sum()
        # Eigen Vector (Normalized) (reduced if reduce is True)
        self.v = self._vn.copy()
        # Eigen Value (reduced if reduce is True)
        self.d = self._d.copy()
