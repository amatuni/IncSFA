...
def testincsfa(animate=False):
    ...
    if PYQT:
        ...
        p1.setrange(0, iterval*t.shape[0]/trainer.ticker)
        p1.setYRange(0, 1.1)

        # Legend
        # args are (size, offset)
        leg = pg.LegendItem(size=(100, 60), offset=(-20, 100))
        # Note we do NOT call plt.addItem in this case
        leg.setParentItem(p1.graphicsItem())

        # init curves
        curves1 = [pg.PlotCurveItem(pen=(i, output_dim*1.3))
        ...
        p21.setrange(0, t.shape[0])
        p21.setYRange(-2, 2)
        p22 = l2.addPlot()
        p22.addItem(curves2[1])
        p22.setrange(0, t.shape[0])
        p22.setYRange(-2, 2)
        l2.nextRow()
        p23 = l2.addPlot()
        p23.addItem(curves2[2])
        p23.setrange(0, t.shape[0])
        p23.setYRange(-2, 2)
        p24 = l2.addPlot()
        p24.addItem(curves2[3])
        p24.setrange(0, t.shape[0])
        p24.setYRange(-2, 2)
        l2.nextRow()
        ...
        if animate:
            view.show()
            trn = trainer.itrain(input_data, iterval=iterval,
            ...
            def animloop():
                ...
                if not rtnval:
                    ...
                    print('Success status: ', np.allclose(
                ...
                for dim in range(output_dim):
                    dcosines[cnt[0], dim] = np.abs(
                        np.dot(v[dim], bv[dim].T))/(np.linalg.norm(v[dim])*np.linalg.norm(bv[dim]))
            ...
            _tcnt = time.time()
            trainer.train(input_data, iterval=iterval,
                          monitor_keys=['slowFeatures'])
            v = trainer.monitorVar['slowFeatures']
            dcosines = np.zeros([len(v), output_dim])
            for i in range(len(v)):
                for dim in range(output_dim):
                    dcosines[i, dim] = np.abs(np.dot(
                        v[i][dim], bv[dim].T))/(np.linalg.norm(v[i][dim])*np.linalg.norm(bv[dim]))
            ...
            print('Total Time for %d iterations: ' %
                  (iterval), time.time()-_tcnt)
            print('Success status: ', np.allclose(
                dcosines[-1], np.ones(output_dim), atol=1e-2))
            print('Maximum error: ', np.max(
