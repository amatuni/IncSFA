    ...
    **Inputs**
    ...
    def __init__(self, ulnode, mode='Incremental', **kwargs):
        ...
        if not(self.mode in self.ulnode._validTrainingModes):
            raise Exception('Invalid training mode! Valid training modes: %s' % (
    ...
    def _dataIncGen(self, x, iterval):
        ...
        for n in range(totSamples):
            updateProgressBar((n+1)/float(totSamples), 'Training %s' %
                              (type(self.ulnode).__name__), self._barLength)
            i = n % x.shape[0]
            yield x[i:i+1]

    def _dataBlkIncGen(self, x, iterval):
        totBlks = x.shape[0]/int(self.blkSize) + \
            (x.shape[0] % int(self.blkSize) > 0)
        totBlks *= iterval
        for blk in range(totBlks):
            updateProgressBar((blk+1)/float(totBlks), 'Training %s' %
                              (type(self.ulnode).__name__), self._barLength)
    ...
    def _dataBatGen(self, x, iterval):
        ...
        for n in range(iterval):
            updateProgressBar((n+1)/float(iterval), 'Training %s' %
                              (type(self.ulnode).__name__), self._barLength)
            yield x
